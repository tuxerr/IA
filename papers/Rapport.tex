...
Ces trois types d'agents ont des similarités, dans le sens où leur tâche 
principale peut-être décrite de la façon suivante, aller chercher une 
ressource et la ramener. De même que dans le cas de la chasse l'emplacement 
peut être préalablement indiquée par le chef, dans le cas contraire il faut 
rechercher la ressource. Ensuite le comportement dépendra de la ressource. 
Dans le cas de l'eau on ne considère que l'eau des lacs, pas celle de la mer, 
cette ressource est considéré comme étant inépuisable et n'entraîne donc de 
diminution d'un stock.
