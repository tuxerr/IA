...
traverser une cellule occupée par un bâtiment.
	...
	\subsection{La technologie utilisée et les sources}

Le langage utilisé est le Python dans sa version 3.3. L'interface graphique a été réalisé avec la bibliothéque Qt version 4.9.
Il est possible de les télécharger et de les instaler, il vous suffis de trouver la dernier release de ces versions sur leurs sites officiel,
puis de lancer main.py comme tout programme Python.\\
Mais ça ne sera pas utile. Il exist dans le fichier build deux dossiers comprenant 
chancun un exécutable, main.exe. L'un des dossiers est pour du 32 bits et l'autre pour 64 bits. Nous avons testé sur plus de cinq machines
win7 est l'exécutable marche très bien. Donc sauf si vous voulez re-éxécuter le code, il vous suffis de double cliquer sur le main.exe qui convient
à votre machine, l'application prend un peu de temps pour se lancer.\\
Dans le dossier principale avec comme extension ".py" sont les sources, et dans le dossier resource il y a les images utilisées pour l'interface 
homme machine.
